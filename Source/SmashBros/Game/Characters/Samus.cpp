...
namespace SmashBros
{
	...
	Samus::GroundStab::GroundStab(byte playerNo, float x1, float y1) : Projectile(playerNo, x1,y1)
	{
		...
		anim->addFrame("Images/Game/Characters/Samus/down_blast.png");
		addAnimation(anim);
...
Samus::Bomb::Bomb(float x1, float y1) : Item(x1, y1, 20000, TYPE_HOLD)
	{
		...
		anim->addFrame("Images/Game/Characters/Samus/bomb.png");
		addAnimation(anim);
		...
		anim->addFrame("Images/Game/Characters/Samus/bomb_explode.png");
		addAnimation(anim);
	...
	void Samus::Bomb::onAnimationFinish(const String&name)
	{
	...
	void Samus::Bomb::Update(long gameTime)
	{
		if(exploded)
		{
			xvelocity = 0;
	...
	boolean Samus::Bomb::use(byte attackDir)
	{
		active = true;
		toss(attackDir);
		return true;
	...
	boolean Samus::Bomb::allowsGrabDelay()
	{
		return active;
	}

	void Samus::Bomb::onPickUp(Player*collide)
	{
		active = false;
		setHitState(false);
	}

	void Samus::Bomb::onDiscard(Player*discarder)
	{
		active = true;
		setHitState(true);
	}

	void Samus::Bomb::onPlayerHit(Player*collide, byte dir)
	{
		if(!isHeld())
		{
	...
	void Samus::Bomb::onPlatformCollide(Platform*collide, byte dir)
	{
		grounded = true;
