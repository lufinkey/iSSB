...
namespace SmashBros
{
	DreamLand::DreamLand(float x1, float y1) : Stage(x1, y1)
	{
		setSpawnPoint(0, 0,0);
		...
		setBackground("Images/Game/Stages/DreamLand/bg.jpg");
		...
		MusicManager::loadSong("Dreamland");
	}
	...
	void DreamLand::loadGameElements()
	{
		GameElement*bg;
		
		bg = new GameElement(0,30);
		Animation*anim = new Animation("normal", 1,1,1);
		anim->addFrame("Images/Game/Stages/DreamLand/mainstage.png");
		bg->addAnimation(anim);
		bg->changeAnimation("normal", FORWARD);
		addElement(bg);
		
		bg = new GameElement(0,-70);
		bg->addAnimation(new Animation("normal",1,"Images/Game/Stages/DreamLand/plat_middle.png"));
		bg->changeAnimation("normal", FORWARD);
		addElement(bg);
		
		bg = new GameElement(-110, -2);
		bg->addAnimation(new Animation("normal",1,"Images/Game/Stages/DreamLand/plat_side.png"));
		bg->changeAnimation("normal", FORWARD);
		addElement(bg);
		
		bg = new GameElement(110, -2);
		anim = new Animation("normal",1,"Images/Game/Stages/DreamLand/plat_side.png");
		anim->mirror(true);
		bg->addAnimation(anim);
		bg->changeAnimation("normal", FORWARD);
		addElement(bg);
	...
	void DreamLand::loadPlatforms()
	{
		Platform*plat;
		
		plat = new Platform(Platform::TYPE_NORMAL, 0,112);
		plat->addAnimation(new Animation("normal",1,"Images/Game/Stages/DreamLand/main_plat.png"));
	...
	void DreamLand::Draw(Graphics2D&g, long gameTime)
	{
